...
public class Particle implements Entity2D {
    ...
    public Particle(float x, float y, float v, float angle) {
        this(nextId++, x, y, v, angle);
    }

    // Explicit ID constructor (used when moving/updating particles)
    public Particle(int id, float x, float y, float v, float angle) {
        if (x < 0 || y < 0) {
    ...
    public float euclideanDistance(Entity2D other) {
        ...
        return (float) Math.hypot(dx, dy) - other.getR();
    }
    ...
    @Override
    public Particle move(float deltaTime, float newAngle) {
        float nextX = x + v * (float) Math.cos(angle) * deltaTime;
        float nextY = y + v * (float) Math.sin(angle) * deltaTime;
        return new Particle(this.id, nextX, nextY, v, newAngle);
    }

    @Override
    public Particle move(float deltaTime, float newAngle, float contourLength) {
    ...
    @Override public float getMinX() { return x; }
    @Override public float getMaxX() { return x; }
    @Override public float getMinY() { return y; }
    @Override public float getMaxY() { return y; }
    @Override public float getR() { return 0.0f; }
